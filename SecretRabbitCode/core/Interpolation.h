#ifndef SRC_INTERPOLATION_H
#define SRC_INTERPOLATION_H
                              ...
                              int available, int wrap, FloatType gain) noexcept
{
    ...
    else
    {
        for (int i = numOut; --i >= 0;)
        {
            ...
            pos -= actualRatio;
            *out++ += gain
                        * InterpolatorType::valueAtOffset (lastInputSamples,
                                                           jmax (ValueConstants<FloatType>::zero, ValueConstants<FloatType>::one - (FloatType) pos));
        }
    }
